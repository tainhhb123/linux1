...
int sort_in_reverse = 0;           // -r  // Đã thêm cho sắp xếp ngược
...
{
    int option;
    while ((option = getopt(argc, argv, "abcdfghiklmnopqrstuvw:xABCDFGHI:LNQRST:UXZ1r")) != -1) // Đã thêm tùy chọn 'r'
    {
        switch (option)
        {
        case 'a':
        ...
        case 'S':
            ...
            break;
        case 'r':
            sort_in_reverse = 1;  // Đặt cờ đảo ngược
            break;
        default:
            fprintf(stderr, "Tùy chọn không hợp lệ\n");
            exit(EXIT_FAILURE);
        }
    }
}